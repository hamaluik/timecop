...
class Timers {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Timers[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Timers[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Timers> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <Timers>[];
  ...
  static Map<String, List<Timers>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Timers.listFromJson(
          entry.value,
          growable: growable,
        );
      }
